...
struct DispatchConstants final
{
    std::uint32_t width{};
    std::uint32_t height{};
    std::uint32_t samplesPerDispatch{};
...
static_assert(sizeof(DisplayConstants) == 12U);
...
[[nodiscard]] lgp::framework::Status ValidateExtent(lgp::framework::Extent2D size)
{
    if (size.width == 0U || size.height == 0U || size.width > kMaximumWidth || size.height > kMaximumHeight)
    {
        return std::unexpected(
            lgp::framework::MakeError("ValidateExtent", "Chapter 16 requires a non-empty extent up to 640x360."));
    }
    return {};
}
                                                                  ...
                                                                  std::wstring_view name, bool mapPersistently)
{
    if (sizeInBytes == 0U)
    {
        return std::unexpected(lgp::framework::MakeError("CreateBuffer", "Chapter 16 buffers must be non-empty."));
    }
    ...
    if (FAILED(result))
    {
        return std::unexpected(lgp::framework::MakeHResultError("ID3D12Device10::CreateCommittedResource3", result,
                                                                "Failed to create a Chapter 16 buffer."));
    }
    ...
    if (!name.empty())
    {
        ...
        if (FAILED(nameResult))
        {
            return std::unexpected(lgp::framework::MakeHResultError("ID3D12Object::SetName", nameResult,
                                                                    "Failed to name a Chapter 16 buffer."));
        }
    }
    ...
    if (mapPersistently)
    {
        ...
        if (FAILED(mapResult))
        {
            return std::unexpected(lgp::framework::MakeHResultError("ID3D12Resource::Map", mapResult,
                                                                    "Failed to map a Chapter 16 readback buffer."));
        }
...
RendererCore::RendererCore(std::filesystem::path shaderPath, bool progressive)
    : shaderPath_(std::move(shaderPath)), progressive_(progressive)
{
}
    ...
    if (FAILED(computeCreate))
    {
        return std::unexpected(
            lgp::framework::MakeHResultError("ID3D12Device::CreateRootSignature", computeCreate,
                                             "Failed to create the Chapter 16 compute root signature."));
    }
    ...
    if (FAILED(graphicsCreate))
    {
        return std::unexpected(
            lgp::framework::MakeHResultError("ID3D12Device::CreateRootSignature", graphicsCreate,
                                             "Failed to create the Chapter 16 graphics root signature."));
    }
    return {};
}
...
lgp::framework::Status RendererCore::CreatePipelines()
{
    ...
    if (FAILED(computeResult))
    {
        return std::unexpected(lgp::framework::MakeHResultError("ID3D12Device::CreateComputePipelineState",
                                                                computeResult,
                                                                "Failed to create the Chapter 16 sampling pipeline."));
    }
    ...
    if (FAILED(graphicsResult))
    {
        return std::unexpected(lgp::framework::MakeHResultError("ID3D12Device::CreateGraphicsPipelineState",
                                                                graphicsResult,
                                                                "Failed to create the Chapter 16 display pipeline."));
    }
    return {};
}
...
lgp::framework::Status RendererCore::CreateFrameSlotResources(lgp::framework::Extent2D size)
{
    ...
    std::uint64_t const momentsBytes = pixelCount * sizeof(float) * 4U;
    ...
    for (FrameSlotResources &slot : frameSlots_)
    {
        ...
        auto moments = CreateBuffer(*deviceResources_->device(), momentsBytes, D3D12_HEAP_TYPE_DEFAULT,
                                    D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS, L"Ch16 Online Moments");
        auto statistics = CreateBuffer(*deviceResources_->device(), statisticsBytes, D3D12_HEAP_TYPE_DEFAULT,
                                       D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS, L"Ch16 Pixel Statistics");
        auto readback = CreateBuffer(*deviceResources_->device(), statisticsBytes, D3D12_HEAP_TYPE_READBACK,
                                     D3D12_RESOURCE_FLAG_NONE, L"Ch16 Statistics Readback", true);
        if (!descriptors)
        {
        ...
        momentsUav.Buffer.StructureByteStride = sizeof(float) * 4U;
...
LabConfiguration RendererCore::ActiveConfiguration() const noexcept
{
    LabConfiguration configuration =
        headless_ && headlessConfiguration_.has_value() ? *headlessConfiguration_ : interactiveConfiguration_;
    configuration.exponent = std::clamp(configuration.exponent, 0.0F, 64.0F);
    configuration.samplesPerDispatch =
        progressive_ ? std::clamp(configuration.samplesPerDispatch, 1U, kMaximumSamplesPerDispatch) : 1U;
...
lgp::framework::Status RendererCore::Update(lgp::framework::UpdateContext const &context)
{
    if (!headless_)
    {
        ...
        {
            interactiveConfiguration_.debugView = DebugView::Estimate;
        }
        ...
        {
            interactiveConfiguration_.debugView = DebugView::StandardError;
        }
        if (context.input.WasKeyPressed('4'))
        {
            interactiveConfiguration_.debugView = DebugView::SampleCount;
        }
...
lgp::framework::Status RendererCore::Render(lgp::framework::FrameContext const &frameContext)
{
    ...
    {
        return std::unexpected(lgp::framework::MakeError("Render", "The Chapter 16 frame slot is out of range."));
    }

    LabConfiguration const configuration = ActiveConfiguration();
    FrameSlotResources &slot = frameSlots_[frameContext.frameSlot];
    bool reset = !slot.initialized || slot.exponent != configuration.exponent ||
                 slot.resetGeneration != resetGeneration_ || configuration.resetAccumulation || !progressive_;
    if (slot.sampleCount > std::numeric_limits<std::uint32_t>::max() - configuration.samplesPerDispatch)
    {
        reset = true;
    }
    std::uint32_t const previousSampleCount = reset ? 0U : slot.sampleCount;
    ...
    BufferBarrierState const before = slot.initialized ? CopySourceState() : NoAccessState();
    std::vector<D3D12_BUFFER_BARRIER> bufferBarriers{
        MakeBufferBarrier(*slot.moments.Get(), before, ComputeUnorderedAccessState()),
        MakeBufferBarrier(*slot.statistics.Get(), before, ComputeUnorderedAccessState()),
    };
    ...
    bufferBarriers = {
        MakeBufferBarrier(*slot.moments.Get(), ComputeUnorderedAccessState(), CopySourceState()),
    ...
    DisplayConstants const display{size_.width, size_.height, static_cast<std::uint32_t>(configuration.debugView)};
    ...
    slot.sampleCount = previousSampleCount + configuration.samplesPerDispatch;
    slot.exponent = configuration.exponent;
...
{
    if (deviceResources_ == nullptr || frameSlots_.empty())
    {
        return std::unexpected(
            lgp::framework::MakeError("ReadBackOutputs", "Chapter 16 has no rendered frame to read back."));
    }

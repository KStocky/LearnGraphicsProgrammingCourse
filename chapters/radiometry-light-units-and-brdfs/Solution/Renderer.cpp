...
inline constexpr std::uint32_t kGridColumns = 5U;
inline constexpr std::uint32_t kGridRows = 4U;
...
struct LightingConstants final
{
    ...
    std::uint32_t useMaterialOverride{};
...
{
    return std::filesystem::path{__FILE__}.parent_path() / "DirectLighting.hlsl";
}
...
[[nodiscard]] DirectX::XMFLOAT3 GridBaseColor(std::uint32_t row) noexcept
{
    constexpr std::array<DirectX::XMFLOAT3, kGridRows> colors{{
        {0.78F, 0.11F, 0.055F},
        {0.08F, 0.42F, 0.82F},
        {0.16F, 0.68F, 0.22F},
        {0.92F, 0.64F, 0.12F},
...
class Renderer::Impl final
{
  public:
    ...
    Visualization visualization{Visualization::Final};
...
lgp::framework::Status Renderer::Impl::Initialize(lgp::framework::ApplicationInitContext const &context)
{
    ...
    auto planeResult = GeneratePlane(18.0F, 14.0F, 1U, 1U);
    ...
    auto dsvResult = lgp::framework::CreateDescriptorHeap(*deviceResources->device(), D3D12_DESCRIPTOR_HEAP_TYPE_DSV,
                                                          1U, false, L"Ch05 depth descriptors");
    if (!dsvResult)
    {
    ...
    if (auto status = CreateMeshBuffers(*sphereResult, L"Ch05 sphere", sphere); !status)
    {
        return status;
    }
    if (auto status = CreateMeshBuffers(*planeResult, L"Ch05 plane", plane); !status)
    {
        return status;
    }
...
lgp::framework::Status Renderer::Impl::Update(lgp::framework::UpdateContext const &context)
{
    ...
    lighting.useMaterialOverride = materialOverride ? 1U : 0U;
    lighting.directionToLight = NormalizeDirection(lightDirection);
    lighting.intensity = lightType == LightType::Directional ? directionalLux : pointCandela;
    ...
    if (headless && headlessTest.has_value())
    {
        ...
        lighting.lightType = static_cast<std::uint32_t>(test.lightType);
        ...
        lighting.lightPosition = {
            test.pointLightPosition.x,
            test.pointLightPosition.y,
            test.pointLightPosition.z,
        };
        lighting.lightColor = {test.lightColor.r, test.lightColor.g, test.lightColor.b};
        lighting.intensity = test.intensity;
...
lgp::framework::Status Renderer::Impl::BuildUi(lgp::framework::UpdateContext const &context)
{
    ...
    ImGui::Begin("Direct-light BRDF laboratory");
    int light = static_cast<int>(lightType);
    ImGui::Combo("Light type", &light, "Directional\0Point\0");
    lightType = static_cast<LightType>(light);
    ...
    int mode = static_cast<int>(visualization);
    ImGui::Combo("Visualization", &mode,
                 "Final shading\0Irradiance\0N dot L\0Diffuse\0Specular\0Fresnel\0GGX distribution\0Smith "
                 "geometry\0Energy warning\0");
    visualization = static_cast<Visualization>(mode);
    ImGui::Separator();
    ImGui::Checkbox("Override every material", &materialOverride);
    ...
    ImGui::Separator();
    ImGui::TextWrapped("Columns increase roughness. Rows change metalness and base color. Drag with the right mouse "
                       "button to orbit; use the wheel to dolly.");
    ImGui::End();
    return {};
}
...
lgp::framework::Status Renderer::Impl::Render(lgp::framework::FrameContext const &frameContext)
{
    ...
    if (headless && headlessTest.has_value())
    {
        ...
        object.baseColor = {test.material.baseColor.r, test.material.baseColor.g, test.material.baseColor.b};
        object.roughness = test.material.roughness;
        object.metalness = test.material.metalness;
        object.dielectricF0 = {
            test.material.dielectricF0.r,
            test.material.dielectricF0.g,
            test.material.dielectricF0.b,
        };
    ...
    else
    {
        for (std::uint32_t row = 0U; row < kGridRows; ++row)
        {
            ...
            {
                float const x = (static_cast<float>(column) - 2.0F) * 1.75F;
                float const z = (static_cast<float>(row) - 1.5F) * 1.75F;
                object.translation = {x, 0.75F, z};
                object.baseColor = GridBaseColor(row);
                object.roughness = 0.08F + (0.84F * static_cast<float>(column) / static_cast<float>(kGridColumns - 1U));
                object.metalness = static_cast<float>(row) / static_cast<float>(kGridRows - 1U);
                object.dielectricF0 = {0.04F, 0.04F, 0.04F};
                DrawMesh(commandList, sphere, object);
            }
        }
